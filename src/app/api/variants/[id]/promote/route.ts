...
export async function POST(
  ...
  try {
    ...
    try { execSync(`git tag -d pre-promote`, { cwd: projectDir, encoding: "utf-8" }); } catch { /* tag may not exist */ }
    execSync(`git tag pre-promote`, { cwd: projectDir, encoding: "utf-8" });
    ...
    if (status) {
      ...
      execSync(`git commit -m "promote: ${branchName}"`, { cwd: projectDir, encoding: "utf-8" });
    }
  } catch (e: any) {
